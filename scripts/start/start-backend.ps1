# Start Backend - Clean and Reliable Script
# Avvia Backend - Script Pulito e Affidabile
...
if (Test-Path "C:\maven\apache-maven-3.9.5\bin\mvn.cmd") {
    ...
    Write-Host "⚠️ Maven non trovato in percorso standard, uso: $mavenCmd" -ForegroundColor Yellow
}
...
Write-Host "`n🚀 Avvio in corso..." -ForegroundColor Green
...
try {
    ...
    Write-Host "`n❌ Errore durante avvio: $_" -ForegroundColor Red
} finally {
